...
class Users extends \App\Pages\Base
{
    ...
    public function __construct() {
        ...
        $plist->filterform->add(new DropDownChoice('typeuser', array(User::ROLE_VENDOR => "Vendor", User::ROLE_CLUBBER => "Clubber", User::ROLE_ORGANIZER => "Organizer")));

        $this->_ds = new EntityDataSource("\\App\\Entity\\User", "userrole not like '%" . User::ROLE_ADMIN . "%'", "username asc");
    ...
    public function OnAddUserRow(\Zippy\Html\DataList\DataRow $datarow) {
        ...
        $datarow->add(new \Zippy\Html\Label("role", $item->rolename()));
    }
    ...
    public function filterOnSubmit($sender) {
        ...
        $role = $this->plist->filterform->typeuser->getValue();
        if ($role > 0) {
            $where .= " and userrole like '%" . $role . "%'";
        }
